...
# )
# cur = con.cursor()
# cur.execute("CREATE DATABASE so")
# cur.execute("CREATE TABLE emp3(name VARCHAR(255) , account VARCHAR(255) , amount VARCHAR(255) , ifsc VARCHAR(255))")
def register():
...
def save():
    con = connect(db='so', user='root', passwd='Suryamani@2', host='localhost')
    # cur.execute("CREATE DATABASE so")
    cur = con.cursor()
    ...
    insert_into = "INSERT INTO emp3 (name , account , amount , ifsc) VALUES(%s , %s, %s, %s)"
    val = (name, account, amount, ifsc)
    i = cur.execute(insert_into, val)
    if i >= 1:
        con.commit()
